drop function if exists indicator_set_euros;

create or replace function indicator_set_euros (
...
begin
    ...
    for var_rec_day in
        select distinct
            to_char ((json ->> 'fecha') :: date, 'yyyymmdd') :: numeric "idf_euro",
    ...
    for var_rec_day in
        select
            idf_euro,
            to_char (to_date (idf_euro :: text, 'yyyymmdd') - '1 day' :: interval, 'yyyymmdd') :: numeric "num_previous",
            num_value
        from
            dat_euros
        where
